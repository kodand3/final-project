...
int main() {
    std::cout << "[child3] PID: " << GetCurrentProcessId() << "\n";
    Sleep(500);

    STARTUPINFOA si = { sizeof(si) };
    PROCESS_INFORMATION pi;
    if (CreateProcessA(NULL, (LPSTR)"notepad.exe", NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi)) {
        std::cout << "[child3] Launched notepad.exe [PID: " << pi.dwProcessId << "]\n";
        Sleep(500);
